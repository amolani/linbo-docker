...
echo "─────────────────────────────────────────────────────────────────"
db_check=$(docker exec linbo-db pg_isready -U linbo 2>/dev/null)
if [ $? -eq 0 ]; then
    echo -e "${GREEN}PostgreSQL is ready${NC}"
    # Count records
    users=$(docker exec linbo-db psql -U linbo -d linbo -t -c "SELECT COUNT(*) FROM users;" 2>/dev/null | tr -d ' ')
    hosts=$(docker exec linbo-db psql -U linbo -d linbo -t -c "SELECT COUNT(*) FROM hosts;" 2>/dev/null | tr -d ' ')
    configs=$(docker exec linbo-db psql -U linbo -d linbo -t -c "SELECT COUNT(*) FROM configs;" 2>/dev/null | tr -d ' ')
    echo "  Users: $users | Hosts: $hosts | Configs: $configs"
else
