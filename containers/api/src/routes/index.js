 ...
 *
 * In sync mode (sync_enabled=true in Redis or SYNC_ENABLED=true env):
...
async function createRouter() {
  ...
  // Determine sync mode from Redis setting (primary) or env var (fallback)
  let isSyncMode = process.env.SYNC_ENABLED === 'true';
  try {
    const settings = require('../services/settings.service');
    const syncSetting = await settings.get('sync_enabled');
    if (syncSetting === 'true') isSyncMode = true;
  } catch {
    // Redis not ready or settings unavailable — fall back to env
  }
