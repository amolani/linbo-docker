...
async function generateMainGrub(hosts, configs, options = {}) {
  ...
  if (pxeHosts.length > 0) {
    ...
    for (const host of pxeHosts) {
      ...
      const groupName = host.hostgroup;

      const kopts = `quiet splash server=${server} group=${groupName} hostgroup=${groupName}`;
  ...
  const defaultGroup = defaultConfig ? `group=${defaultConfig.id} hostgroup=${defaultConfig.id}` : '';
