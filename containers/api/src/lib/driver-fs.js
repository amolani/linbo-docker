...
/**
 * Generate driver-manifest.json for a patchclass directory
 * @param {string} pcDir - Patchclass directory path
 * @param {string} mapHash - Pre-computed hash of driver-map.json
 * @returns {Promise<object>} The manifest object
 */
async function generateManifest(pcDir, mapHash) {
  ...
  const manifestPath = path.join(pcDir, 'driver-manifest.json');
