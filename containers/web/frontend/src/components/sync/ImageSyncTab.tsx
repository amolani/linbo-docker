...
import { Download, RefreshCw, X, Loader2, HardDrive, CheckCircle, AlertTriangle, Clock } from 'lucide-react';
...
export function ImageSyncTab() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [comparison, queueData] = await Promise.all([
  ...
  const handlePullImage = async (imageName: string) => {
    try {
      ...
      const msg = error instanceof Error ? error.message : 'Fehler beim Starten';
                  ...
                  return (
                    <tr key={img.name} className="hover:bg-accent/50">
                      ...
                      <td className="px-6 py-4 text-sm text-muted-foreground">
                        {img.remote ? formatBytes(Number(img.remote.imagesize || img.remote.totalSize)) : '-'}
                      </td>
                      <td className="px-6 py-4 text-sm text-muted-foreground">
